...
const {
  SNSClient, ListTopicsCommand, PublishCommand, CreateTopicCommand
} = require('@aws-sdk/client-sns');
const {
  S3Client, ListObjectsV2Command, GetObjectCommand, PutObjectCommand, CreateBucketCommand, HeadBucketCommand
...
async function ensureTopicArn() {
  ...
  const found = (list.Topics || []).find(t => (t.TopicArn || '').endsWith(TOPIC_SUFFIX));
  if (found) return found.TopicArn;
  const created = await sns.send(new CreateTopicCommand({ Name: TOPIC_SUFFIX.split(':').pop() }));
...
// traza completa
app.get('/trace/:id', async (req, resa) => {
  const id = req.params.id;
  const child = await listKeys(`traces/${id}/`);
  const keys = child.sort();
  const steps = [];
  for (const k of keys) {
    const data = await getJsonOr(k, null);
    steps.push({ key: k, data });
  }
  res.json({ id, steps });
});
...
app.get('/domain/metrics', async (_req,res)=>{
  ...
  for (const k of keys) {
    try {
      ...
      const cur = agg.get(id) || { productId:id, unitsSold:0, totalRevenue:0 };
