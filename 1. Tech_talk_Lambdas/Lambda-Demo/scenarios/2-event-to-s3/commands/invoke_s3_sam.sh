...
EVENT_FILE="$DIR/../events/post-request.json"
ENV_VARS="$DIR/../commands/env/s3_env.json"
...
echo '☁️ Invocando a la Lambda ApiToS3Function para guardar archivo en S3...'

sam local invoke ApiToS3Function \
  --template "$TEMPLATE" \
  --event "$EVENT_FILE" \
  --env-vars "$ENV_VARS"
